...
def _normalize_sections(raw_sections: list["NoteSection"]) -> list[NoteSection]:
    ...
    Each section's markdown is also delimiter-normalized (`convert_delimiters`)
    so a model that slipped into `\\[…\\]` / `\\(…\\)` despite the prompt still
    persists canonical `$`/`$$` math (issue #33).
    """
    return [
        NoteSection(
            heading=(sec.heading or "").strip(),
            markdown=convert_delimiters((sec.markdown or "").strip()),
...
async def _single_pass_synthesis(
    ...
    try:
        ...
        # Deterministically canonicalize delimiters before persisting: even with
        # the `$`/`$$`-only prompt, the model occasionally emits `\[…\]`/`\(…\)`,
        # which remark-math renders raw. `convert_delimiters` is pure/idempotent,
        # so valid `$`-math is untouched (issue #33).
        markdown = convert_delimiters((out.markdown or "").strip())
...
async def _synthesize_segment(
    ...
    if not source:
        return None
    out = await _run_synthesis_agent(source, instructions)
    # Belt-and-suspenders against per-segment delimiter drift (issue #33): the
    # prompt forbids `\[…\]`/`\(…\)`, but normalize deterministically too so a
    # slipped segment can't reach the stitched doc.
    body = convert_delimiters((out.markdown or "").strip())
    if not body:
        return None
    section = NoteSection(
        heading=label,
...
async def _segmented_synthesis(
    ...
    # The `ordered` sections are already delimiter-normalized (per-segment), so
    # the stitch is too; normalize once more on the joined doc as a cheap final
    # guard so the persisted flat `markdown` can never carry `\[…\]` (issue #33).
    flat_markdown = convert_delimiters(_stitch_markdown(ordered))
    if log is not None:
        await log.info(
